...
const GamePage = () => {
  ...
  const winSumArr = useMemo(() => gameDataBase.map((q) => q.winSum), [
    gameDataBase,
  ]);
  const questionObjArr = useMemo(() => gameDataBase.map((q) => q.question), [
    gameDataBase,
  ]);
  ...
  return (
    <div className="game-container">
      ...
      {winWidth < 960 && showMenu && (
        <MenuBox winSumArr={winSumArr} setShowMenu={setShowMenu} />
      )}
